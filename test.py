...
for i in range(times):
    ...
    x_max = 10*np.ones(d)
    x_min = -10*np.ones(d)
    optimizer = PSO(fit_func=Schwefel_P222,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = PSO(fit_func=Quadric,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = PSO(fit_func=Schwefel_P221,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 30*np.ones(d)
    x_min = -30*np.ones(d)
    optimizer = PSO(fit_func=Rosenbrock,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    optimizer = PSO(fit_func=Quadric_Noise,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 500*np.ones(d)
    x_min = -500*np.ones(d)
    optimizer = PSO(fit_func=Schwefel,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    optimizer = PSO(fit_func=Rastrigin,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 600*np.ones(d)
    x_min = -600*np.ones(d)
    optimizer = PSO(fit_func=Griewank,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 50*np.ones(d)
    x_min = -50*np.ones(d)
    optimizer = PSO(fit_func=Generalized_Penalized01,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 50*np.ones(d)
    x_min = -50*np.ones(d)
    optimizer = PSO(fit_func=Generalized_Penalized02,
                    num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    optimizer = PSO(fit_func=DE_JONG_N5,
    ...
    x_max = 5*np.ones(4)
    x_min = -5*np.ones(4)
    optimizer = PSO(fit_func=Kowalik,
    ...
    x_max = 5*np.ones(2)
    x_min = -5*np.ones(2)
    optimizer = PSO(fit_func=Six_Hump_Camel,
                    num_dim=2, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 2*np.ones(2)
    x_min = -2*np.ones(2)
    optimizer = PSO(fit_func=Goldstein_Price,
                    num_dim=2, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 1*np.ones(3)
    x_min = 0*np.ones(3)
    optimizer = PSO(fit_func=Hartmann_3D,
    ...
    x_max = 1*np.ones(6)
    x_min = 0*np.ones(6)
    optimizer = PSO(fit_func=Hartmann_6D,
    ...
    x_max = 10*np.ones(4)
    x_min = 0*np.ones(4)
    optimizer = PSO(fit_func=Shekel_m5,
                    num_dim=4, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 10*np.ones(4)
    x_min = 0*np.ones(4)
    optimizer = PSO(fit_func=Shekel_m7,
                    num_dim=4, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 10*np.ones(4)
    x_min = 0*np.ones(4)
    optimizer = PSO(fit_func=Shekel_m10,
                    num_dim=4, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
